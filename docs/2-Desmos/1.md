...
4. By default, the inserted image is placed behind any graphs and is not set to "draggable".
   ...
   and we can zoom in or out by using the wheel of a mouse or two fingers on a trackpad.
...
- **On the left panel, click on an empty entry, type** `Y ~ aX^2→ + bX + c`**, and hit `ENTER`**,
  ...
  - The `~` is called *tilde* and is often found
    ...
    You need to use the `shift` key to enter it.
...
## [Problem solved](https://www.desmos.com/calculator/hcth3cqh1w){:target="_blank"}
...
Can you think of a good name for these collection of questions?
