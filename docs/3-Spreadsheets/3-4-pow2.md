...
## The text datatype versus the number datatype

Before progressing any further,
it is useful to highlight some differences
between the two dataypes: *text* and *number*.
...
you are about to see that computers are often far-from-perfect at math!**
 ...
 - In cell `F49`,
   type `Hello, cell!`.
   - A little read triangle appears at the top-right.
 ...
 - In cell `F51`,
   ...
   - Upon hovering over it, you receive a message saying,
     "Invalid: This value does not match the column type **number**".
   - This is confusing!
     Why is 1125899906842624 not regarded
     as a number in this context??
 ...
 - In cell `F51`,
   ...
   - It is confusing that
     the previous number led to problems
     and that this number does not.
 ...
 - Cells `F51`, `F53`, and `F55` all demonstrate the following.
   - **Google Sheets will only let you specify a number
...
from some *text*.
 - When the *text* is `123456.75`,
   using `VALUE` produces a *number* that stores 123456.75.
 - When the text is `Hello, cell!`, using `VALUE` gives an error.
 - When the text is `12345678901234567890`, using `VALUE` gives an error
   because 12345678901234567890 has more than 15 significant figures.
